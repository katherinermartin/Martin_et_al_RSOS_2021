...
# Subset to the alleles that occur in 10 or more greens with FP and the columns of sample_ID and paps_texture (0s and 1s) but include Chmy33 (only occurs in 4 C. mydas with FP), since it had very high relative importance in the random forest model.
Cm_data <- subset(Cm_data, select = c(sample_ID, paps_texture, Chmy02, Chmy01, Chmy10, Chmy04, Chmy05, Chmy33))


Cm_data[Cm_data$paps_texture == 1, "paps_texture"] <- "smooth" # replace 1s with smooth texture
Cm_data[Cm_data$paps_texture == 0, "paps_texture"] <- "rough" # replace 0s with rough texture
...
barplot(tab2_HA_Chmy33, beside = TRUE, main = "FP texture in turtles with and without Chmy33 allele, HA correction", legend.text = c("Chmy33 +", "Chmy33 -"))
...
ggplot(data=Cm_risks, aes(x=allele, y=est, ymin=lower, ymax=upper)) +
  ...
  xlab("allele") + ylab("relative risk estimate of FP texture (95% CI)") +
