# risk ratios of top alleles and tumor regression, in C. mydas with FP
...
# Subset to the alleles that occur in 10 or more greens with FP and the columns of sample_ID and paps_regressed (0s and 1s) but include Chmy33 (only occurs in 4 C. mydas with FP), since it had very high relative importance in the random forest model.
Cm_data <- subset(Cm_data, select = c(sample_ID, paps_regressed, Chmy02, Chmy01, Chmy10, Chmy04, Chmy05, Chmy33))


Cm_data[Cm_data$paps_regressed == 1, "paps_regressed"] <- "regressed" # replace 1s with regressed and 0s with not regressed for pap regression state
Cm_data[Cm_data$paps_regressed == 0, "paps_regressed"] <- "active"
...
tab_Chmy01 <- table(Chmy01, paps_regressed) # independent, dependent
tab_Chmy01

# rearrange so it's in ABCD format
tab2_Chmy01 <- cbind(tab_Chmy01[,2], tab_Chmy01[,1]) # rearrange columns
colnames(tab2_Chmy01) <- c("regressed", "active")
...
barplot(tab2_Chmy01, beside = TRUE, main = "FP regression in turtles with and without Chmy01 allele", legend.text = c("Chmy01 +", "Chmy01 -"))
...
p_Chmy01 <- as.data.frame(Chmy01_risk$massoc$chisq.strata)

# Chmy02
Cm_data[Cm_data$Chmy02 == 1, "Chmy02"] <- "Chmy02 +"
Cm_data[Cm_data$Chmy02 == 0, "Chmy02"] <- "Chmy02 -"

tab_Chmy02 <- table(Chmy02, paps_regressed) # independent, dependent
tab_Chmy02

# rearrange so it's in ABCD format
tab2_Chmy02 <- cbind(tab_Chmy02[,2], tab_Chmy02[,1]) # rearrange columns
colnames(tab2_Chmy02) <- c("regressed", "active")
...
barplot(tab2_Chmy02, beside = TRUE, main = "FP regression in turtles with and without Chmy02 allele", legend.text = c("Chmy02 +", "Chmy02 -"))
...
tab_Chmy04 <- table(Chmy04, paps_regressed) # independent, dependent
tab_Chmy04

# rearrange so it's in ABCD format
tab2_Chmy04 <- cbind(tab_Chmy04[,2], tab_Chmy04[,1]) # rearrange columns
colnames(tab2_Chmy04) <- c("regressed", "active")
...
barplot(tab2_Chmy04, beside = TRUE, main = "FP regression in turtles with and without Chmy04 allele", legend.text = c("Chmy04 +", "Chmy04 -"))
...
p_Chmy04 <- as.data.frame(Chmy04_risk$massoc$chisq.strata)
...
tab_Chmy05 <- table(Chmy05, paps_regressed) # independent, dependent
tab_Chmy05

# rearrange so it's in ABCD format
tab2_Chmy05 <- cbind(tab_Chmy05[,2], tab_Chmy05[,1]) # rearrange columns
colnames(tab2_Chmy05) <- c("regressed", "active")
...
barplot(tab2_Chmy05, beside = TRUE, main = "FP regression in turtles with and without Chmy05 allele", legend.text = c("Chmy05 +", "Chmy05 -"))
...
tab_Chmy10 <- table(Chmy10, paps_regressed) # independent, dependent
tab_Chmy10

# rearrange so it's in ABCD format
tab2_Chmy10 <- cbind(tab_Chmy10[,2], tab_Chmy10[,1]) # rearrange columns
colnames(tab2_Chmy10) <- c("regressed", "active")
...
barplot(tab2_Chmy10, beside = TRUE, main = "FP regression in turtles with and without Chmy10 allele", legend.text = c("Chmy10 +", "Chmy10 -"))
...
tab_Chmy33 <- table(Chmy33, paps_regressed) # independent, dependent
tab_Chmy33

# rearrange so it's in ABCD format
tab2_Chmy33 <- cbind(tab_Chmy33[,2], tab_Chmy33[,1]) # rearrange columns
colnames(tab2_Chmy33) <- c("regressed", "active")
...
barplot(tab2_Chmy33, beside = TRUE, main = "FP regression in turtles with and without Chmy33 allele", legend.text = c("Chmy33 +", "Chmy33 -"))
...
tab2_HA_Chmy33
colnames(tab2_HA_Chmy33) <- c("regressed", "active")
...
HA_Chmy33_risk <- epi.2by2(tab2_HA_Chmy33, method = "cohort.count", conf.level = 0.95)

HA_Chmy33_risk

df_HA_Chmy33 <- as.data.frame(HA_Chmy33_risk$massoc$RR.strata.wald)
...
Cm_row_names <- read.csv("../Data/top alleles/Cm_regressed_top_alleles.csv")
...
ggplot(data=risk_ratio_regressed_top_Cm_alleles, aes(x=allele, y=est, ymin=lower, ymax=upper)) +
  ...
  xlab("allele") + ylab("relative risk estimate of FP regression (95% CI)") +
