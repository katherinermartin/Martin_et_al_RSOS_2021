# Risk ratio of top alleles and tumor regression in C. mydas with FP, with Bonferroni correction
...
# Subset to the alleles that occur in 10 or more greens with FP and the columns of sample_ID and paps_regressed (0s and 1s) but include Chmy33 (only occurs in 4 greens with FP), since it had very high relative importance in the random forest model.
...
ggplot(data=risk_ratio_regressed_top_Cm_alleles, aes(x=allele, y=est, ymin=lower, ymax=upper)) +
  ...
  xlab("allele") + ylab("relative risk estimate of FP regression (99.2% CI)") +
