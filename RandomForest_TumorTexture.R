...
length(which(data$paps_smooth_regressed==1)) # 29 C. mydas with smooth/regressed tumor texture for 28.3% regression.
...
# there are 30 smooth/regressed tumor individuals; 29*(2/3) = 19.3 individuals, so we'll sample 19 smooth/regressed tumor individuals and 19 rough tumor individuals for the training data set so that the resutling tree forest is not biased.
...
print(rf_default) # mtry = 9.695 at 70.75472% accuracy.
...
print(rf_gridsearch)
plot(rf_gridsearch) # most accurate: mtry 22 at 74.52830%

# Now begin the full Random Forest analyses: going to use mtry = 9.695 (default), and mtry = 22; 22  was given from grid search tuning with decently high accuracy.
...
cor(importance_rf_default_a,importance_rf_default_b) # A correlation of 0.997091 for predictor importance values between forests when mtry = 9.695 and ntree = 10,000

# forests with mtry = 22

rf_22_a <- randomForest(x = data[,2:96], y = as.factor(data$paps_smooth_regressed), importance=TRUE ,proximity=TRUE, mtry=11, ntree=10000, strata=as.factor(data$paps_smooth_regressed), sampsize=sample_size)

rf_22_b <- randomForest(x = data[,2:96], y = as.factor(data$paps_smooth_regressed), importance=TRUE ,proximity=TRUE, mtry=11, ntree=10000, strata=as.factor(data$paps_smooth_regressed), sampsize=sample_size)

#Check correlation of locus importance values between forests 
importance_rf_22_a <- data.frame(importance(rf_22_a,type=1)) #type=1 is mean decrease in accuracy for classification, so a large, positive value means that permuting the variable led to a big decrease in prediction accuracy (which is indicative of an important locus)
colnames(importance_rf_22_a)<-c("importance")

importance_rf_22_b <- data.frame(importance(rf_22_b,type=1))
colnames(importance_rf_22_b)<-c("importance")

cor(importance_rf_22_a,importance_rf_22_b) # A correlation of 0.9974304 for predictor importance values between forests when mtry = 28 and ntree = 10,000

# mtry = 22 had a slightly higher importance value correlation, so run both mtry = 22 and mtry = default

# Build final model with mtry = 22, ntree = 10,000. 
...
data.model = randomForest(paps_smooth_regressed ~ .,
                          ...
                          importance=TRUE,
                          proximity=TRUE,
                          mtry=22, # based on tuning done above
...
sum(diag(table(Prediction = data.predict,Truth = data.test$paps_smooth_regressed))) / sum(table(Prediction = data.predict,Truth = data.test$paps_smooth_regressed)) # Accuracy of model with all variables: 63.51351%
...
data.model_defaultmtry <- randomForest(paps_smooth_regressed ~ .,
                                       ...
                                       proximity=TRUE,
                                       ntree=10000, # based on tuning for ntree above
                                       strata=as.factor(data$paps_smooth_regressed), sampsize=sample_size) # take into account imbalance in FP postiives vs FP negatives
data.model_defaultmtry # OOB-ER = 34.38%
...
sum(diag(table(Prediction = data.predict_defaultmtry,Truth = data.test$paps_smooth_regressed))) / sum(table(Prediction = data.predict_defaultmtry,Truth = data.test$paps_smooth_regressed)) # Accuracy of model with all variables: #66.21622%
